...
def merge_data(existing_data, new_subjects):
    ...
    for new_subj in new_subjects:
        ...
        if subj_id in existing_map:
            ...
            existing_subj["phong_hoc"] = new_subj["phong_hoc"]
            ...
            # KHÔNG cập nhật thoi_gian và trang_thai (admin có thể đã sửa)
...
def scrape_dlu():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            page.wait_for_selector("input[type='text'], input[name='username'], input[placeholder*='Mã']", timeout=10000)
            ...
            all_cookies = page.context.cookies(["https://online.dlu.edu.vn", "https://portal-api.dlu.edu.vn"])
            ...
            for namhoc, hocky in semesters:
                ...
                weeks_url = f"{api_base}/api/student/WeekSchedule?namhoc={namhoc}&hocky={hocky}"
                
                try:
                    weeks_resp = session.get(weeks_url, timeout=15)
                    if weeks_resp.status_code != 200:
                        print(f"LỖI: Không lấy được tuần cho {namhoc} {hocky}. Status: {weeks_resp.status_code}")
                        continue
                        ...
                        for week_num in unique_weeks:
                            ...
                            try:
                                ...
                                if resp.status_code == 200:
                                    ...
                                    print(f"    LỖI 401 ở tuần {week_num} - auth hết hạn!")
                                    return False
            ...
            # === BƯỚC 7: Lọc bỏ ngày Thứ 2-6, chỉ giữ Thứ 7 + Chủ Nhật ===
            all_raw_data = [item for item in all_raw_data if item.get("DayOfWeek", 0) >= 6]
            print(f"Sau khi lọc bỏ ngày thường (Thứ 2-6): {len(all_raw_data)} entries cuối tuần.")
